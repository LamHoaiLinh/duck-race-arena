...
const GAME_CONFIG = {
  ...
  rankShuffleMinGap: 4.0,
  rankShuffleMaxGap: 6.0,
  rankShuffleMinPlaces: 1,
  rankShuffleMaxPlaces: 2,
...
const MAP_DATA = [
  ...
  },
  {
    id: 'skybridge-knot',
    name: 'Skybridge Knot ★★',
    description: 'Nút giao ba tầng với hai cầu vượt và một đoạn hầm chui ở giữa.',
    bgA: '#CDEFE5', bgB: '#F2FFFB', accent: '#6FC4AE',
    allowCrossings: true,
    trackWidthFactor: 0.82,
    structures: [
      { type: 'overpass', progress: 0.18, span: 0.042 },
      { type: 'tunnel', progress: 0.47, span: 0.032 },
      { type: 'overpass', progress: 0.73, span: 0.040 }
    ],
    points: [[0.10,0.68],[0.20,0.22],[0.43,0.18],[0.71,0.74],[0.88,0.36],[0.72,0.18],[0.45,0.78],[0.22,0.82],[0.12,0.47],[0.39,0.50],[0.67,0.47],[0.88,0.76],[0.58,0.87],[0.30,0.31]]
  },
  {
    id: 'subway-serpent',
    name: 'Subway Serpent ★★',
    description: 'Đường rắn tự cắt, có hai cửa hầm và một cầu nổi chạy chéo qua trung tâm.',
    bgA: '#D7EEE7', bgB: '#F5FFFB', accent: '#7FB9A7',
    allowCrossings: true,
    trackWidthFactor: 0.80,
    structures: [
      { type: 'tunnel', progress: 0.12, span: 0.034 },
      { type: 'overpass', progress: 0.39, span: 0.046 },
      { type: 'tunnel', progress: 0.66, span: 0.034 }
    ],
    points: [[0.09,0.76],[0.18,0.20],[0.48,0.24],[0.78,0.80],[0.91,0.48],[0.75,0.19],[0.47,0.73],[0.22,0.84],[0.10,0.50],[0.34,0.39],[0.66,0.61],[0.88,0.30],[0.60,0.12],[0.31,0.66]]
  },
  {
    id: 'infinity-stack',
    name: 'Infinity Stack ★★',
    description: 'Hai vòng vô cực xếp chồng, liên tục chuyển từ cầu vượt sang hầm chui.',
    bgA: '#D8F2EA', bgB: '#F7FFFC', accent: '#84C8B2',
    allowCrossings: true,
    trackWidthFactor: 0.78,
    structures: [
      { type: 'overpass', progress: 0.10, span: 0.040 },
      { type: 'tunnel', progress: 0.34, span: 0.030 },
      { type: 'overpass', progress: 0.58, span: 0.042 },
      { type: 'tunnel', progress: 0.82, span: 0.030 }
    ],
    points: [[0.10,0.58],[0.22,0.18],[0.48,0.20],[0.72,0.82],[0.90,0.58],[0.77,0.20],[0.51,0.79],[0.25,0.22],[0.10,0.48],[0.31,0.76],[0.61,0.24],[0.89,0.47],[0.68,0.84],[0.37,0.17]]
  },
  {
    id: 'metro-interchange',
    name: 'Metro Interchange ★★',
    description: 'Nút giao kiểu vành đai đô thị, nhiều nhánh đan xen và cua đổi tầng liên tục.',
    bgA: '#D5EEE8', bgB: '#F4FFFB', accent: '#72B6A8',
    allowCrossings: true,
    trackWidthFactor: 0.78,
    structures: [
      { type: 'overpass', progress: 0.22, span: 0.044 },
      { type: 'tunnel', progress: 0.43, span: 0.032 },
      { type: 'overpass', progress: 0.61, span: 0.044 },
      { type: 'tunnel', progress: 0.87, span: 0.032 }
    ],
    points: [[0.08,0.70],[0.18,0.26],[0.42,0.14],[0.65,0.72],[0.89,0.24],[0.92,0.60],[0.70,0.87],[0.44,0.30],[0.19,0.86],[0.10,0.48],[0.37,0.58],[0.61,0.18],[0.86,0.78],[0.56,0.82],[0.30,0.20]]
  },
  {
    id: 'spiral-flyover',
    name: 'Spiral Flyover ★★',
    description: 'Đường xoắn ốc mở rộng ra ngoài, có cầu vượt kép và hầm cong ở đoạn cuối.',
    bgA: '#D3F0E3', bgB: '#F3FFF9', accent: '#79C39D',
    allowCrossings: true,
    trackWidthFactor: 0.80,
    structures: [
      { type: 'overpass', progress: 0.16, span: 0.048 },
      { type: 'overpass', progress: 0.48, span: 0.044 },
      { type: 'tunnel', progress: 0.76, span: 0.036 }
    ],
    points: [[0.11,0.73],[0.13,0.28],[0.42,0.13],[0.78,0.25],[0.88,0.60],[0.68,0.86],[0.36,0.78],[0.24,0.51],[0.43,0.31],[0.69,0.42],[0.63,0.68],[0.42,0.62],[0.52,0.46],[0.81,0.79],[0.90,0.37]]
  },
  {
    id: 'dragonfly-cross',
    name: 'Dragonfly Cross ★★',
    description: 'Hình chuồn chuồn bốn cánh, trung tâm giao cắt dày với cầu và hầm xen kẽ.',
    bgA: '#D8F5EA', bgB: '#F6FFFC', accent: '#78CDB4',
    allowCrossings: true,
    trackWidthFactor: 0.76,
    structures: [
      { type: 'tunnel', progress: 0.09, span: 0.032 },
      { type: 'overpass', progress: 0.29, span: 0.044 },
      { type: 'tunnel', progress: 0.53, span: 0.032 },
      { type: 'overpass', progress: 0.77, span: 0.044 }
    ],
    points: [[0.09,0.55],[0.20,0.18],[0.46,0.38],[0.73,0.14],[0.91,0.46],[0.69,0.58],[0.88,0.84],[0.54,0.70],[0.28,0.88],[0.10,0.66],[0.31,0.48],[0.13,0.25],[0.47,0.18],[0.66,0.46],[0.43,0.83],[0.22,0.62]]
  }
];
...
const EVENT_COMMENTS = {
  ...
  rankSurge: '{name} tăng tốc bất ngờ và đổi thứ hạng!'
};
...
function buildTrackGeometry(width, height, laneCount, map) {
  ...
  const simplePath = map.allowCrossings ? pathPoints : untangleClosedPath(pathPoints);
  const smoothingPasses = Number.isFinite(map.smoothingPasses) ? map.smoothingPasses : 4;
  const samples = buildClosedChaikin(simplePath, smoothingPasses);
  ...
  const trackWidthFactor = Number.isFinite(map.trackWidthFactor) ? map.trackWidthFactor : 1;
  const trackWidth = clamp(preferredLaneWidth * laneCount * trackWidthFactor, 54, maxTrackWidth * trackWidthFactor);
...
class RaceEngine {
  constructor({ names, duration, laps = GAME_CONFIG.defaultLaps, map, characters, onEvent, onFinish }) {
    ...
    this.nextRankShuffleAt = rand(GAME_CONFIG.rankShuffleMinGap, GAME_CONFIG.rankShuffleMaxGap);
  ...
  updateVisualState(dt) {
    for (const racer of this.racers) {
      ...
      const sprinting = racer.effects.some((effect) => ['finalSprint', 'tailwind', 'jumpSuccess', 'slipstream', 'comebackWind', 'rankSurge'].includes(effect.type) && effect.until > this.elapsed);
  ...
  handleDynamicRankShuffle() {
    const active = this.getRanking().filter((racer) => racer.finishedAt === null);
    if (active.length < 2) return;

    const leader = active[0];
    const safeLeaderProgress = Math.min(leader.progress, this.totalLaps - 0.0001);
    const leaderLapIndex = Math.floor(Math.max(0, safeLeaderProgress));

    // Moi khi sang vong moi, hen mot lan dao hang som hon de vong nao cung co bien dong.
    if (leaderLapIndex !== this.lastObservedLapIndex) {
      this.lastObservedLapIndex = leaderLapIndex;
      this.nextRankShuffleAt = Math.min(this.nextRankShuffleAt, this.elapsed + rand(2.4, 3.8));
    }

    if (this.elapsed < this.nextRankShuffleAt) return;
    this.nextRankShuffleAt = this.elapsed + rand(GAME_CONFIG.rankShuffleMinGap, GAME_CONFIG.rankShuffleMaxGap);

    const lapProgress = safeLeaderProgress - leaderLapIndex;
    const nearLapFinish = lapProgress >= GAME_CONFIG.rankShuffleNearFinishThreshold;
    const nearRaceFinish = this.elapsed >= this.duration * 0.82 || (this.totalLaps - leader.progress) <= 0.24;
    const nearFinish = nearLapFinish || nearRaceFinish;

    // O gan vach dich, 60% lan hen se co dao hang de tao bat ngo.
    if (nearFinish && Math.random() >= GAME_CONFIG.rankShuffleNearFinishChance) return;

    // Chi chon nguoi dang o sau de tang toc vuot len; nhu vay hinh anh luon chay tien,
    // khong bi keo lui tren duong dua. Nguoi bi vuot se tu dong tut 1-2 hang.
    const candidates = [];
    for (let index = 1; index < active.length; index += 1) {
      const racer = active[index];
      if (racer.rankShiftCooldownUntil > this.elapsed) continue;

      const racerLap = Math.floor(Math.max(0, Math.min(racer.progress, this.totalLaps - 0.0001)));
      if (racer.rankShiftLap !== racerLap) {
        racer.rankShiftLap = racerLap;
        racer.rankShiftCount = 0;
      }

      const maxPlaces = Math.min(GAME_CONFIG.rankShuffleMaxPlaces, index);
      for (let places = GAME_CONFIG.rankShuffleMinPlaces; places <= maxPlaces; places += 1) {
        const targetIndex = index - places;
        const target = active[targetIndex];
        const gap = Math.max(0, target.progress - racer.progress);
        // Khong teleport qua khoang cach qua lon. Dua thuong duoc can bang nen gap nay thuong nho.
        if (gap > 0.18) continue;

        let weight = 1;
        weight *= racer.id === this.lastRankShuffleRacerId ? 0.28 : 1;
        weight *= 1 / (1 + racer.rankShiftCount * 0.85);
        if (places === 2) weight *= nearFinish ? 1.40 : 0.72;
        if (nearFinish && targetIndex <= 2) weight *= 1.55;
        if (index === active.length - 1) weight *= 1.12;
        candidates.push({ racer, target, index, targetIndex, places, gap, weight });
      }
    }

    if (!candidates.length) return;
    const choice = weightedPick(candidates.map((item) => ({ value: item, weight: item.weight })));
    if (!choice) return;

    const { racer, target, places } = choice;
    const from = Number.isFinite(racer.displayProgress) ? racer.displayProgress : racer.progress;
    const epsilon = rand(0.006, 0.012);
    const to = Math.min(this.totalLaps - 0.003, Math.max(racer.progress + 0.004, target.progress + epsilon));
    if (to <= racer.progress) return;

    racer.progress = to;
    racer.displayProgress = Math.min(racer.displayProgress, to);
    racer.jumpAnim = {
      from,
      to,
      start: this.elapsed,
      end: this.elapsed + GAME_CONFIG.rankShuffleVisualDuration,
      arcHeight: 0
    };
    racer.rankShiftCount += 1;
    racer.rankShiftCooldownUntil = this.elapsed + GAME_CONFIG.rankShuffleMinGap * 0.85;
    this.lastRankShuffleRacerId = racer.id;
    this.rankShuffleSerial += 1;

    // Dieu chinh pacing nhe de thu hang moi ton tai du lau, khong bi keo ve ngay lap tuc.
    racer.targetFinishTime = Math.max(this.elapsed + 1.0, racer.targetFinishTime - this.duration * (0.010 + places * 0.006));
    this.addEffect(racer, 'rankSurge', 1.07 + places * 0.025 + racer.stats.burst * 0.018, GAME_CONFIG.rankShuffleBoostDuration);
    racer.bubble = {
      text: `+${places} hạng!`,
      type: 'rankSurge',
      until: this.elapsed + Math.max(GAME_CONFIG.bubbleMinTime, 2.0)
    };
    this.createVisualEvent('rankSurge', racer);
    racer.eventHistory.push({ type: 'rankSurge', at: this.elapsed, places });
    this.onEvent?.({
      type: 'rankSurge',
      racer,
      comment: `${racer.name} tăng tốc bất ngờ và vươn lên ${places} hạng!`
    });
  }
  ...
  createVisualEvent(type, racer) {
    ...
    if (type === 'finalSprint' || type === 'slipstream' || type === 'comebackWind' || type === 'rankSurge') visualType = 'speed';
...
class GameApp {
  ...
  drawCircuitTrack(ctx, track, map) {
    ...
    // Ve cau vuot / ham chui cho cac map dac biet.
    this.drawTrackStructures(ctx, track, map);
  ...
  traceTrackSegment(ctx, track, progress, span = 0.04, offset = 0, steps = 28) {
    const start = progress - span / 2;
    ctx.beginPath();
    for (let i = 0; i <= steps; i += 1) {
      const t = i / steps;
      const p = sampleTrack(track, start + span * t);
  ...
  drawTrackStructures(ctx, track, map) {
    if (!Array.isArray(map.structures) || !map.structures.length) return;

    for (const structure of map.structures) {
      const progress = Number.isFinite(structure.progress) ? structure.progress : 0.5;
      const span = Number.isFinite(structure.span) ? structure.span : 0.04;
      if (structure.type === 'overpass') {
        this.drawOverpass(ctx, track, progress, span);
      } else if (structure.type === 'tunnel') {
        this.drawTunnel(ctx, track, progress, span);
      }
    }
  }

  drawOverpass(ctx, track, progress, span) {
    ctx.save();
    ctx.lineJoin = 'round';
    ctx.lineCap = 'round';

    // Bong sau lam ro doan cau nam tren cung.
    ctx.shadowColor = 'rgba(0,0,0,0.36)';
    ctx.shadowBlur = 18;
    ctx.shadowOffsetY = 12;
    this.traceTrackSegment(ctx, track, progress, span, 0);
    ctx.strokeStyle = 'rgba(54,61,66,0.58)';
    ctx.lineWidth = track.trackWidth + 28;
    ctx.stroke();

    ctx.shadowColor = 'transparent';
    this.traceTrackSegment(ctx, track, progress, span, 0);
    ctx.strokeStyle = '#B9BDC2';
    ctx.lineWidth = track.trackWidth + 18;
    ctx.stroke();
    ...
    // Lan lines tren mat cau.
    for (let i = 1; i < track.lanes; i += 1) {
      const offset = -track.trackWidth / 2 + track.laneWidth * i;
      ctx.save();
      ctx.setLineDash([12, 10]);
      this.traceTrackSegment(ctx, track, progress, span, offset);
    ...
    // Thanh chan cau hai ben.
    const edge = track.trackWidth / 2 + 7;
    for (const side of [-1, 1]) {
      this.traceTrackSegment(ctx, track, progress, span, edge * side);
      ctx.strokeStyle = '#DDE4E7';
      ctx.lineWidth = 5;
      ctx.stroke();
      this.traceTrackSegment(ctx, track, progress, span, edge * side);
      ctx.strokeStyle = '#6E777D';
      ctx.lineWidth = 2;
      ctx.stroke();
    }

    const mid = sampleTrack(track, progress);
    ctx.translate(mid.x, mid.y);
    ctx.rotate(Math.atan2(mid.ty, mid.tx));
    ctx.fillStyle = 'rgba(255,255,255,0.92)';
    this.roundRect(ctx, -22, -10, 44, 20, 8);
    ctx.fill();
    ctx.fillStyle = '#355B50';
    ctx.font = '800 9px system-ui, sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('CẦU', 0, 0.5);
    ctx.restore();
  }

  drawTunnel(ctx, track, progress, span) {
    const mid = sampleTrack(track, progress);
    const angle = Math.atan2(mid.ty, mid.tx);
    const portalLength = clamp(track.trackWidth * 0.72, 34, 64);
    const portalWidth = track.trackWidth + 24;

    ctx.save();
    ctx.translate(mid.x, mid.y);
    ctx.rotate(angle);

    // Than ham va vien cua ham.
    ctx.fillStyle = 'rgba(27,38,43,0.46)';
    this.roundRect(ctx, -portalLength / 2 - 7, -portalWidth / 2 - 7, portalLength + 14, portalWidth + 14, 15);
    ctx.fill();
    ctx.fillStyle = '#3E474C';
    this.roundRect(ctx, -portalLength / 2, -portalWidth / 2, portalLength, portalWidth, 12);
    ctx.fill();
    ctx.fillStyle = '#242A2E';
    this.roundRect(ctx, -portalLength / 2 + 5, -track.trackWidth / 2 + 3, portalLength - 10, track.trackWidth - 6, 9);
    ctx.fill();

    // Den vang trong ham.
    ctx.fillStyle = '#FACC15';
    for (let x = -portalLength / 2 + 12; x <= portalLength / 2 - 12; x += 18) {
      ctx.beginPath();
      ctx.arc(x, -track.trackWidth / 2 + 8, 2.5, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(x, track.trackWidth / 2 - 8, 2.5, 0, Math.PI * 2);
      ctx.fill();
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.92)';
    this.roundRect(ctx, -20, -9, 40, 18, 7);
    ctx.fill();
    ctx.fillStyle = '#293B36';
    ctx.font = '800 8px system-ui, sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('HẦM', 0, 0.5);
    ctx.restore();
  }
  ...
  drawRacerSprite(ctx, racer, p, now) {
    ...
    // Chieu cao nhan vat bang xap xi 3 lan be rong cua mot lane.
    // Co gioi han an toan nhe de khong bi vo bo cuc tren mobile rat nho.
    const drawBox = clamp(laneWidth * 3, 36, 96);
